...
import { Clock, Layers, Loader2, Users, Wrench, X } from "lucide-react";
...
export function CriticalEquipmentDetailsDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-[480px] flex-col border-l border-gold/25 bg-[#0a0b0b] text-champagne shadow-[0_0_60px_rgba(0,0,0,0.6)]">
        {/* Cabeçalho */}
        <div className="flex items-start justify-between gap-3 border-b border-gold/20 bg-[#070808] px-5 py-4">
          <div className="min-w-0">
            ...
            {item ? (
              <p className="mt-0.5 text-xs text-zinc-400">{display(item.equipmentCode)}</p>
            ) : null}
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {loading ? (
            ...
            <div className="flex h-40 items-center justify-center px-6 text-center text-sm text-zinc-400">
              {error ?? "Não foi possível carregar os detalhes deste equipamento."}
            </div>
          ) : (
            <div className="space-y-5">
              ...
              <Section title="Responsáveis mais frequentes">
                <Breakdown items={details.responsibleBreakdown} />
              </Section>

              {/* Grupos de planejamento */}
              <Section title="Grupos de planejamento">
                <Breakdown items={details.planningGroupBreakdown} />
              </Section>

              {/* Últimas ordens */}
              <Section title={`Últimas ordens (${details.lastOrders.length})`}>
                <div className="space-y-2">
                  {details.lastOrders.map((order) => (
                    <div
                      key={`${order.osNumber}-${order.operation ?? ""}`}
                      className="rounded-lg border border-gold/15 bg-black/30 px-3 py-2"
                    >
...
function Breakdown({ items }: { items: Array<{ name: string; count: number }> }) {
  if (!items.length) {
    return <p className="text-xs text-zinc-500">Não informado.</p>;
  }
    ...
    <div className="space-y-1.5">
      {items.map((entry) => (
        <div key={entry.name} className="flex items-center justify-between gap-2 text-xs">
          <span className="truncate text-zinc-300" title={entry.name}>
            {entry.name}
          </span>
          <span className="shrink-0 font-semibold text-champagne">{int(entry.count)}</span>
        </div>
      ))}
    </div>
  );
