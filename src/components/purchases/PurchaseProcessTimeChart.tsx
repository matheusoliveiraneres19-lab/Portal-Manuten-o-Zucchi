...
const STAGES = [
  { key: "averageRequisitionToOrderDays", label: "Req. → Pedido", color: CHART_SERIES.ordens },
  { key: "averageOrderToReceiptDays", label: "Pedido → Receb.", color: CHART_SERIES.producao },
  { key: "averageMigoToMiroDays", label: "MIGO → MIRO", color: CHART_SERIES.compras },
  { key: "averageTotalProcessDays", label: "Total", color: GOLD.deep }
] as const;
...
export function PurchaseProcessTimeChart({ times, className = "" }: { times: PurchaseProcessTimes; className?: string }) {
  ...
  return (
    <article className={`panel rounded-lg p-4 ${className}`}>
      <h3 className="text-[11px] font-extrabold uppercase tracking-wide text-gold-deep">Tempo médio do processo (dias)</h3>
