...
/** Envia o arquivo (já validado) para o bucket privado. */
export async function uploadAttachment(path: string, body: Buffer, contentType: string): Promise<void> {
...
export async function removeObject(path: string): Promise<void> {
  const { error } = await getClient().storage.from(ATTACHMENTS_BUCKET).remove([path]);
  if (error) throw error;
}
